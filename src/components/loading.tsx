interface LoadingFallbackProps {
  message?: string;
}

export default function LoadingFallback({ message = 'Loading...' }: LoadingFallbackProps) {
  return (
    <div className="min-h-screen flex flex-col bg-[var(--color-bg)]">
      <header className="relative border-b-2 border-[var(--color-border)] bg-[var(--color-bg-card)]">
        <div className="absolute inset-x-0 top-0 h-0.5 bg-gradient-to-r from-[var(--gradient-start)] to-[var(--gradient-end)]"></div>
        <div className="max-w-5xl mx-auto px-6 py-4">
          <div className="relative pl-8">
            <span className="absolute left-0 top-1/2 -translate-y-1/2 text-[var(--gradient-start)]">◆</span>
            <h1 className="text-xl font-bold tracking-tight text-[var(--color-text-primary)]">
              PayTest
            </h1>
          </div>
        </div>
      </header>
      <main className="flex-1 flex items-center justify-center">
        <p className="animate-pulse text-[var(--color-text-secondary)]">
          {message}
        </p>
      </main>
    </div>
  );
}
